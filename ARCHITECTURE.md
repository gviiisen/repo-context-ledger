...
| Path | Role |
| --- | --- |
| `src/repo_context_ledger/runtime.py.tmpl` | Canonical runtime body and ordered build marker. |
| `src/repo_context_ledger/contracts.pyfrag` | Version, schema, exit, constant, error, and typed result contracts injected at build time. |
...
The builder reads UTF-8 source, normalizes line endings to LF, replaces exactly one explicit marker, emits no timestamp or machine path, and writes outputs atomically. `--check` compares bytes and never writes. CI runs the check on Windows and Ubuntu with Python 3.10 and 3.12 before tests.
