...
| --- | --- |
| `skills/repo-context-ledger/scripts/ledger.py` / `init_repo` | 安装仓库本地运行时并维护受管 Agent 入口。 |
...
- Input: Agent 根据自然语言任务确定 feature，并从统一 Manifest 或 `context --query` 获取相关路径。
- Flow: 原生 Agent 入口指向仓库规则，仓库规则再路由到 Context Pack、稳定 spec 和必要代码；完成或切换时将已验证状态保存为 checkpoint/handoff。
...
- Invariants: `docs/specs/` 是当前事实，`docs/changes/` 是时间历史，Context Pack 是加载路线；工具专用文件不得成为唯一事实源。
