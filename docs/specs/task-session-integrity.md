...
- Output: `context-bundle-v1` returns an additive `resume-capsule-v2` for one accessible match. It preserves summary, next step, Pack-scoped implementation evidence paths, omitted legacy-path count, verification, Git position, Pack, warnings, tool, and epoch while adding goal, current state, next action, explicit Pack anchors, must-preserve contracts, verified facts, unresolved questions, Required reads, and cold-document guidance. Active commands name the selected session and publication target. Verification results use stable local-root placeholders. `--timings` emits one private, non-persisted JSON timing summary to stderr without repository paths. `finish` outputs one completed change path, or preserves the draft and returns an error.
...
- Permissions / concurrency: A repository write lock protects short state, draft, grant, and publication writes. New private session drafts, workspace state, router cache, and preset trust use mode `0600` on POSIX; this local permission boundary is separate from principal-level workflow isolation. External verification commands and finish preparation run without the lock. Ordinary short writers wait for a bounded interval rather than failing on momentary contention. The final finish lock is a compare-and-swap boundary over the session, draft, and bounded inputs; it does not serialize long validation or derived-index work. The principal defaults to the OS account and may use a repository-local configured identity for shared workstations; this is logical isolation, while filesystem permissions remain the security boundary. Agents may not contact or steer another user-owned task unless the user explicitly authorized cross-task coordination.
- Failure / recovery: Missing evidence, a false path, a stale continuation epoch, an expired grant, a foreign access attempt, the current session's stale Pack, a persistent write lock, or inputs changed during finish preparation preserves the draft. Ambiguous resume returns candidates without choosing. If post-publication derived synchronization fails, the completed Change remains authoritative and a later `sync --derived` repairs summaries. A different clone can recover only Git-tracked Packs/specs/completed Changes because private state does not travel. Interrupted publication recovery remains idempotent by Session ID.
- Non-goals: Task sessions do not persist full chats, infer task progress/diffs/symbols, generate translations, guarantee semantic completeness from the Capsule, copy or merge source files, provide filesystem security or distributed locking across machines, synchronize private state through Git, or enforce Codex application messaging permissions at runtime.

## Verification

Run `python -m unittest discover -s tests -p test_ledger.py` to cover v8 migration, same-principal cross-Agent resume, continuation epochs, foreign Capsule isolation, expiring read-only/fork/transfer grants, explicit parallel evidence, concurrent verification, bounded lock waits, finish compare-and-swap rejection, atomic publication, ambiguity, adapter policy, timing privacy, unlocked verification, and local-root redaction. Run `python skills/repo-context-ledger/scripts/ledger.py --repo . check --strict` as the repository-wide integration validation.
