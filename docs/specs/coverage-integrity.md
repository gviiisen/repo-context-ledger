...
| `skills/repo-context-ledger/scripts/ledger.py::derived_only_changes`, `ledger_policy` | 根据真实 Git delta 选择 ordinary/derived-only PR 策略，并聚合团队重叠、Coverage、派生幂等性和 diff 门禁。 |
...
- Flow: `finish` 只校验当前 session。`policy --base` 根据 merge-base delta 自动选择门禁：任何实现、Pack/spec/Change 正文或 README 人工 prose 都进入 ordinary 并运行团队重叠、strict changed-scope Coverage 与 diff check；只有生成索引、Manifest 和 README 受管区块可进入 derived-only，并要求一次内存中的 `sync --derived` 不产生任何计划变更。普通实现路径继续要求 current Pack 精确跟踪；rename 则保留 transition，在 merge base 读取旧路径的 Pack 所有权，在当前工作区读取新路径的 Pack 所有权，并要求两端 feature 一致且相关 Pack 已刷新。无关旧文档债务不进入阻塞错误。指纹对 UTF-8 文本把 CRLF 规范为 LF；Git `-text`、NUL 内容和非 UTF-8 内容保持原始字节。
...
- Output: 增量检查报告 base、实际检查路径数和只属于当前 delta 的确定性错误；全仓检查继续报告全部结构/链接/过期债务；session 门禁只报告当前 evidence 不真实、明确 spec 未记录、遗漏已有相关 Pack 或当前路径指纹过期。测试、CI、配置、生成文件以及 `docs/audit-dispositions/*.json` Ledger 文档不会单独触发行为 Coverage。

## Boundaries and failure modes

- Invariants: `implementation_globs` 至少包含一个规则；glob 必须为非空仓库相对路径且不能包含 `..`；无关 Context Pack 或无关私有 session 不能满足生产路径覆盖；production rename 的旧路径必须由 base Pack 覆盖，新路径必须由同一 feature 的 current Pack 覆盖，且 source/current Pack 迁移必须进入本次 delta；derived-only 资格只由实际内容差异决定，不能由分支名、路径名或手工声明获得；增量检查不能掩盖当前问题，也不能因范围外旧债务失败；LF/CRLF 逻辑等价文本共享既有 `sha256:` 指纹。
...
运行 `python -m unittest discover -s tests -p "test_ledger.py" -v` 验证默认分类、配置兼容、相关 Pack、LF/CRLF、Git binary 属性、显式 session evidence，以及增量检查边界；运行 `python -m unittest discover -s tests -p "test_policy_and_audit.py" -v` 验证真实 derived-only delta、ordinary 回退和派生幂等性。PR 使用 `policy --base origin/main`；定时或 Release 全仓审计使用 `check --strict --coverage`。

<!-- repo-context-ledger:changes:start -->
## Related changes

- [Classify historical dispositions as Ledger documentation](../changes/2026/08/20260828000221-gviiisen-43054cb61c-classify-historical-dispositions-as-ledger-docum.md)
