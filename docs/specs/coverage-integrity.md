...
- Flow: `finish` 只校验当前 session。`check --strict --changed-since` 读取 delta 中的语义文档，并通过一次 current Pack 反向映射展开被修改源码直接关联的 Pack/spec；因此 Pack 文件即使未改，只要其 tracked source 变化导致指纹 stale 仍会失败。无关旧文档债务不进入阻塞错误。省略 `--changed-since` 保持原有全仓扫描。Coverage 继续先分类路径，再把生产路径与 current Context Pack 的精确 tracked file 建立关联。指纹对 UTF-8 文本把 CRLF 规范为 LF；Git `-text`、NUL 内容和非 UTF-8 内容保持原始字节。
...
- Invariants: `implementation_globs` 至少包含一个规则；glob 必须为非空仓库相对路径且不能包含 `..`；无关 Context Pack 或无关私有 session 不能满足生产路径覆盖；增量检查不能掩盖当前 delta 或直接关联 current Pack/spec 的问题，也不能因关联范围外旧债务失败；不带 `--changed-since` 的全仓检查语义不变；LF/CRLF 逻辑等价文本共享既有 `sha256:` 指纹，真实文本变化和二进制字节变化必须 stale。
